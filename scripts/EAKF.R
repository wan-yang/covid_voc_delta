...
EAKF = function(epi.model=epi.model, num_ens=num_ens,inflat=1.03, 
  ...
  for(ihyp in 1:length(hyps)){ # 1:length(hyps)
    ...
    if(hyp.t %in% c('beta.only.minor','both.minor','both.minorb.majors','both.minorb.slows','both.minorb.slows.minor')){ # minor change for beta
      ...
      # SR.bounds.wider.t = SR.bounds.wider
      # 4/14/21 - maybe too strong and overly reply on beta?
    ...
    for (tt in 1:num_times_i){ # num_times_i
      ...
      if(loc.t == 'in') { # for india
        if(as.Date(Week.starts[1]) >= as.Date('2020/3/22')){  # if starts later
          tt.alpha.incr1 = 5:6
          tt.alpha.incr2 = 7:10
          if(tt == 5){
            DA.bounds['alpha',1] = .01 # .02  # reduce uncertainty
            DA.bounds['alpha',2] = pmax(DA.bounds['alpha',2], .2)
            
            SR.bounds['alpha',1] = alpha_bounds2[1]  # update this too b/c started really low
            SR.bounds['alpha',2] = pmax(SRalpha_bounds2[2] * .8, alpha_bounds2[2])
            
            SR.bounds['ifr',] = SRifr_bounds1
            
            print('wk 5 update ifr bound')
          }
        } else if(as.Date(Week.starts[1]) <= as.Date('2020/3/8')){
          tt.alpha.incr1 = 8:9
          tt.alpha.incr2 = 10:12
          if(tt == 8){
            DA.bounds['alpha',1] = .01 # .02   # reduce uncertainty
            DA.bounds['alpha',2] = pmax(DA.bounds['alpha',2], .2)
            ...
            print('wk 10 update ifr bound')
          }
        }
        
        if(tt == 15){
          DA.bounds['alpha',1] = .02 # .02   # reduce uncertainty
      ...
      if(tt == end1stWave  & loc.t %in% c('sa', 'br', 'in')){  
      ...
      if(doSR){
        ...
        # 6/11/21 addition sr on ifr for the India after the first few weeks
        # b/c %SR is only 3% for these weeks
        if(loc.t == 'in' & tt %in% 11:15) {  # do it a few times; help a bit but not critical
          idx = sample(1:num_ens, round(num_ens * .1, 0), replace = F)
        ...
        if(dx.t.newitot[tt] > N * .15/100 & tt>= main2ndWave & (! grepl('s.only', hyp.t)) &
          ...
          # else {
          #   cntSRwave2 = cntSRwave2 + p.tmp.raw
          #   print(paste('cntSRwave2 = ', round(cntSRwave2, 2)))
          # }
        ...
        if(tt>= main2ndWave & mn.po > N * .1/100){
          ...
          # cntSRwave2 = cntSRwave2 + abs(dx.t.newitot[tt]) / pmin(mn.po, N * .5/100) + 
          #   abs(dx.t.newiobs[tt]) / pmin(obs_i[tt,1], N * .05/100) + 
          #   abs(dx.t.e[tt]) / pmin(e.po, N * .3/100) + abs(dx.t.i[tt]) / pmin(i.po, N * .3/100)
