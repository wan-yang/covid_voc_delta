...
write.csv(tab, paste0(dir_res, 'tab_voc.b1617.summary.est.csv'), row.names = F)
...
tmp = cumIperc_ens[eval == 'obs.more' & date == '2020-05-17']
summary(tmp[,-c(1:3)]%>% unlist)
(tmp[,-c(1:3)]%>% unlist) %>% quantile(prob=c(.025,.975))
(tmp[,-c(1:3)]%>% unlist) %>% mean
tmp = cumIperc_ens[eval == 'obs.more' & date == '2020-08-16']
summary(tmp[,-c(1:3)]%>% unlist)
tmp = cumIperc_ens[eval == 'obs.more' & date == '2020-12-27']
summary(tmp[,-c(1:3)]%>% unlist)
tmp = cumIperc_ens[eval == 'obs.more' & date == '2021-01-31']
(tmp[,-c(1:3)]%>% unlist) %>% quantile(prob=c(.025,.975))
(tmp[,-c(1:3)]%>% unlist) %>% mean
# get projections
source(paste0(dir_code,'getCombinedRes_projections.R'))
save(res.proj, file = paste0(dir_res, 'res.proj.RData'))
