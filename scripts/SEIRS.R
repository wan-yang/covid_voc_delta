...
# include vaccination 
# v1: consider possible high ve after 1 dose for those with prior infection
SEIRSV = function(tm_strt, tm_end, tm_step=1, # 1 day time-step
                 ...
                 VE1priorInf, VE2priorInf, # Vaccine efficacy, for those with prior infection
                 cum.dS.po, # number of people had prior infection but lost their immunity due to the immune evasive variant
                 cum.ImmLoss # number of people had prior infection but lost their immunity due to waning
){
  ...
  for(tt in tm_vec){
    ...
    if(diffVEforPriorInf){ # consider diff ve for those with prior infection
      # !!! CAUTION, ONLY do this for variants that evade prior immunity
      # b/c for non-immune evasive variants, infected/recovered are already removed from the S pool
      # could base this on cum.dS.perc (changes in S due to immune evasion)
      # this represent the percentage prior immunity lost against the new variant, but they may still boost imm after 1 dose of vacc
      # cum.dS.po is the total number of people lose their prior immunity and return to the S pool due to imm evasion
      # so these are the people may have higher Ve after first dose
      # what is the prob of the new vacc'ed are in this camp?
      ## first prob of one gets infected before the rise of new variant, this is recoded in stat.sr.S (weeks that S is probed)
      # cumIwt = N - xpost['S1',,stat.sr.S[1]$week]  # cumulative infection before the immune evasive variant
      # (cumIwt / N) * (cum.dS.po / cumIwt) = cum.dS.po / N
      # also consider those lost immunity due to waning, those could be boosted more easily as well
      n.priorInf = cum.dS.po + cum.ImmLoss
      pPriorInf = pmin(1, pmax(0, n.priorInf / N))  # the prob of the new vacc'ed had prior infection but lost their immunity b/c the immune evasive variant
      # note: if cum.dS.po == 0 (i.e. pPriorInf == 0), the formula would be same as no diff in VE for those with prior infection but lost their immunity b/c the immune evasive variant
      
      vacc.d1 = V1[cnt-1,] * pPriorInf * percS *  VE1priorInf + # those with prior non-specific immunity but lost it due to delta
        V1[cnt-1,] * (1-pPriorInf) * percS * VE1 # those w/o prior non-specific immunity
      
      VE2acc = 1-(1-VE2)/(1-VE1)
      if(VE1priorInf==1){
        VE2acc.priorInf = 0
      } else {
        VE2acc.priorInf = 1-(1-VE2priorInf)/(1-VE1priorInf)
      }
      
      vacc.d2 = V2[cnt-1,] * pPriorInf * percS *  (1 - VE1priorInf) * VE2acc.priorInf +
        V2[cnt-1,] * (1-pPriorInf) * percS * (1 - VE1) * VE2acc
...
SEIRSV_v0 = function(tm_strt, tm_end, tm_step=1, # 1 day time-step
                  tmstep = tmstep,
                  state0 = state0,
                  S0, E0, I0, # H0, # N0,
                  beta, Tei, Tir, Trs,
